 ...
 ***********************************************************************
 *
 *  File name     : jack_module_stereo.cpp
 *  System name   : jack_module_stereo
 *
...
JackModuleStereo::JackModuleStereo()
{
...
JackModuleStereo::JackModuleStereo(unsigned long inbufsize, unsigned long outbufsize)
{
...
int JackModuleStereo::init()
{
    return init("JackModuleStereo");
} // init()

int JackModuleStereo::init(std::string clientName)
{
    /*
...
int JackModuleStereo::_wrap_jack_process_cb(jack_nframes_t nframes, void *arg)
{
    return ((JackModuleStereo *)arg)->onProcess(nframes);
 ...
 *
 */
int JackModuleStereo::onProcess(jack_nframes_t nframes)
{
    /*
     *    (audio input)
    ...
    if (frames_pushed < nframes * numberOfInputChannels)
    ...
    if (frames_popped < nframes * numberOfOutputChannels)
 ...
 *
 */
int JackModuleStereo::setNumberOfInputChannels(int n)
{
...
int JackModuleStereo::setNumberOfOutputChannels(int n)
{
...
unsigned long JackModuleStereo::getSamplerate()
{
    return jack_get_sample_rate(client);
} // getSamplerate()

void JackModuleStereo::autoConnect()
{
    autoConnect("system", "system");
} // autoConnect()

void JackModuleStereo::autoConnect(std::string inputClient, std::string outputClient)
{
    /*
    ...
    if (numberOfOutputChannels > 0)
    {
        ...
        for (int channel = 0; channel < numberOfOutputChannels; channel++)
        {
            ...
            if (nrofoutputports > 0)
                outputportindex %= nrofoutputports;
        } // for channel
...
unsigned long JackModuleStereo::readSamples(float *ptr, unsigned long nrofsamples)
{
    // pop samples from JACK inputbuffer and hand over to the caller
    return inputringbuffer->pop(ptr, nrofsamples);
} // readSamples()

unsigned long JackModuleStereo::writeSamples(float *ptr, unsigned long nrofsamples)
{
